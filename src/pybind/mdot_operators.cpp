...
pydblocs_type py_single_operator_real(std::string name) {
  index_t nb_blocs;
  ...
  if (name == "sh_id_u1") {
    ...
    throw std::invalid_argument(
        std::string("The single operator (blocs) \"") + name +
        std::string(
            "\" is not available.\n"
            "\t\t- Available operators are "
            "sh_id_u1, sh_sp_u1, sh_sm_u1, sh_sx_u1, "
            "sh_sz_u1, sh_id_no, sh_sp_no, sh_sm_no, sh_sx_no, sh_sz_no, ..."));
  }
...
pyzblocs_type py_single_operator_cplx(std::string name) {
  index_t nb_blocs;
  std::vector<std::tuple<index_t, index_t>> indices;
  std::vector<index_t> sizes;
  std::vector<std::tuple<index_t, index_t>> shapes;
  ...
  if (name == "sh_id-cplx_u1") {
    ...
    throw std::invalid_argument(
        ...
        std::string("\" is not available.\n"
                    "\t\t- Available operators are "
                    "sh_id-cplx_u1, sh_sy_u1, sh_id-cplx_no, sh_sy_no, ..."));
  }
